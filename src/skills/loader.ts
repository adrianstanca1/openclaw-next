...
import { SKILL_ERRORS, BUILTIN_SKILLS, SYSTEM_SKILL_PREFIX } from './index.js';
...
export interface SkillLoaderConfig {
  /**
   * Base path for local skill installation
   */
  localSkillPath?: string;

  /**
   * Base path for bundled skills
   */
   ...
   */
  remoteRegistries?: RemoteRegistryConfig[];
...
export interface RemoteRegistryConfig {
  ...
  /**
   * Whether this is a fallback registry
   */
  fallback?: boolean;
}

/**
 * Skill metadata cache entry
 */
interface SkillCacheEntry {
  skill: SkillDefinition | MarketplaceSkill;
  loadedAt: string;
  ttl: number;
  source: SkillSource;
}
...
export class SkillLoader {
   ...
   * Loaded skills cache
   */
  private readonly skillCache: Map<string, SkillCacheEntry> = new Map();

  /**
   * Known skills by ID
   */
  private readonly knownSkills: Map<string, SkillDefinition> = new Map();
   ...
   */
  async loadLocalSkill(filePath: string): Promise<SkillDefinition> {
    try {
   ...
   */
  async loadLocalSkills(dirPath: string): Promise<SkillDefinition[]> {
   ...
   * @returns Loaded skill definition
   */
  async loadBundledSkill(packagePath: string): Promise<SkillDefinition> {
    try {
      ...
      if (!this.isDirectory(packagePath)) {
        throw new SkillLoaderError(
          `Invalid bundled skill path: ${packagePath}`,
          SKILL_ERRORS.NOT_FOUND
        );
      }
      ...
      throw new SkillLoaderError(
        `No skill definition found in bundled package: ${packagePath}`,
        SKILL_ERRORS.NOT_FOUND
      );
    } catch (error) {
      if (error instanceof SkillLoaderError) {
      ...
      throw new SkillLoaderError(
        `Failed to load bundled skill from ${packagePath}: ${
          (error as Error).message
        }`,
        SKILL_ERRORS.INSTALL_FAILED
      );
   ...
   */
  private async loadBundledTarball(packagePath: string): Promise<SkillDefinition> {
    // In production, this would extract and load the tarball
    // For now, return a placeholder
    // A proper implementation would use 'tar' or 'zlib' modules

    throw new SkillLoaderError(
      `Tarball extraction not yet implemented: ${packagePath}`,
      'NOT_IMPLEMENTED'
    );
  }
   ...
   * @returns Loaded skill definition
   */
  async loadRemoteSkill(skillId: string, registry?: RemoteRegistryConfig): Promise<SkillDefinition> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new SkillLoaderError(
          `Failed to fetch skill from registry: ${response.statusText}`,
          SKILL_ERRORS.NETWORK_ERROR
        );
      }

      const data = await response.json();
      const definition = data as SkillDefinition;
      ...
      throw new SkillLoaderError(
        `Failed to load remote skill ${skillId}: ${(error as Error).message}`,
        SKILL_ERRORS.NETWORK_ERROR
      );
    }
  }
  ...
  async searchRemoteSkills(
    ...
    for (const registry of this.config.remoteRegistries || []) {
      try {
        ...
        const data = await response.json();
        const skills = data as MarketplaceSkill[];
        results.push(...skills);
      } catch (error) {
        // Log error but continue with other registries
        console.warn(
          `Failed to search registry ${registry.name}: ${(error as Error).message}`
        );
      }
    }
   ...
   */
  loadBuiltinSkill(skillId: string): SkillDefinition {
    const definition = this.getBuiltinSkillDefinition(skillId);
    if (!definition) {
      throw new SkillLoaderError(
        `Unknown builtin skill: ${skillId}`,
        SKILL_ERRORS.NOT_FOUND
      );
    }
   ...
   */
  private getBuiltinSkillDefinition(skillId: string): SkillDefinition | undefined {
   ...
   */
  getCachedSkill(skillId: string): SkillDefinition | MarketplaceSkill | undefined {
   ...
   */
  private isCacheExpired(entry: SkillCacheEntry): boolean {
  ...
  private validateSkillDefinition(definition: SkillDefinition): void {
    ...
    if (errors.length > 0) {
      throw new SkillLoaderError(
        `Invalid skill definition: ${errors.join('; ')}`,
        SKILL_ERRORS.INVALID_DEFINITION
      );
    }
  }
   ...
   */
  async acquireSkill(skillId: string): Promise<boolean> {
   ...
   */
  private readDirectory(path: string): string[] {
 ...
 */
export async function loadSkill(identifier: string): Promise<SkillDefinition> {
