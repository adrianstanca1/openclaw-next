...
export type { InstalledPlugin } from './types.js';
...
export class PluginRegistry {
   ...
   */
  private definitions: Map<string, PluginDefinition>;

  /**
   * Map of installed plugins by instance ID
   */
  private instances: Map<string, InstalledPlugin>;

  /**
   * Map of plugin instances by plugin ID (for multiple versions)
   */
  private instancesByPluginId: Map<string, Set<string>>;
   ...
   */
  constructor() {
    this.definitions = new Map<string, PluginDefinition>();
    this.instances = new Map<string, InstalledPlugin>();
    this.instancesByPluginId = new Map<string, Set<string>>();
    this.dependencies = new Map<string, Set<string>>();
    this.reverseDependencies = new Map<string, Set<string>>();
  }
  ...
  registerDefinition(definition: PluginDefinition): string {
    ...
    if (definition.requires && definition.requires.length > 0) {
      ...
      for (const dep of definition.requires) {
        const reverse = this.reverseDependencies.get(dep.id) || new Set<string>();
   ...
   */
  unregisterDefinition(pluginId: string): boolean {
  ...
  installInstance(
    ...
    if (!pluginInstances) {
      pluginInstances = new Set<string>();
   ...
   */
  getInstance(instanceId: string): InstalledPlugin | undefined {
   ...
   * @returns The installed plugin or undefined
   */
  getInstanceByPluginId(pluginId: string): InstalledPlugin | undefined {
   ...
   */
  getInstancesByPluginId(pluginId: string): InstalledPlugin[] {
    const instances = this.instancesByPluginId.get(pluginId);
    if (!instances) {
      return [];
    }
   ...
   */
  getAllInstances(): InstalledPlugin[] {
   ...
   */
  getInstancesByState(state: PluginState): InstalledPlugin[] {
   ...
   */
  getEnabledInstances(): InstalledPlugin[] {
   ...
   */
  getDisabledInstances(): InstalledPlugin[] {
   ...
   */
  getErroredInstances(): InstalledPlugin[] {
   ...
   */
  updateInstanceState(instanceId: string, newState: PluginState): boolean {
    const instance = this.instances.get(instanceId);
    if (!instance) {
      return false;
    }
   ...
   */
  updateInstanceConfig(instanceId: string, config: Record<string, unknown>): boolean {
    const instance = this.instances.get(instanceId);
    if (!instance) {
      return false;
    }
   ...
   */
  setInstanceError(instanceId: string, error: PluginError): void {
    const instance = this.instances.get(instanceId);
    if (instance) {
      instance.lastError = {
        ...error,
   ...
   */
  incrementExecutionCount(instanceId: string): number {
    const instance = this.instances.get(instanceId);
    if (!instance) {
      return 0;
    }
   ...
   */
  checkDependenciesSatisfied(instance: InstalledPlugin): boolean {
   ...
   */
  findDependencies(pluginId: string): string[] {
   ...
   */
  getPluginsWithMissingDependencies(): InstalledPlugin[] {
    return Array.from(this.instances.values()).filter(
      inst => !this.checkDependenciesSatisfied(inst)
    );
  }
   ...
   */
  getPluginsBySource(source: PluginSource): InstalledPlugin[] {
  ...
  getSummary(): {
    ...
    for (const source of ['local', 'remote', 'bundled', 'marketplace', 'builtin'] as PluginSource[]) {
 ...
 * @returns The plugin ID
 */
export const registerPlugin = (definition: PluginDefinition): string => {
 ...
 * @returns The plugin definition or undefined
 */
export const getPluginDefinition = (pluginId: string): PluginDefinition | undefined => {
 ...
 * @returns The installed plugin or undefined
 */
export const getPluginInstance = (instanceId: string): InstalledPlugin | undefined => {
 ...
 * @returns Array of all installed plugin instances
 */
export const getAllPlugins = (): InstalledPlugin[] => {
 ...
 */
export const getPluginsWithMissingDeps = (): InstalledPlugin[] => {
