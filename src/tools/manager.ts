...
import { ToolExecutor, toolExecutor } from './executor';
...
export class ToolManager {
  private policies: Map<ToolId | ToolCategory, ToolPolicy>;
  private configOverrides: Map<ToolId, ToolConfigUpdate>;

  constructor() {
    this.policies = new Map<ToolId | ToolCategory, ToolPolicy>();
    this.configOverrides = new Map<ToolId, ToolConfigUpdate>();
  }
   ...
   */
  enableTools(toolIds: ToolId[]): number {
    let count = 0;
   ...
   */
  disableTools(toolIds: ToolId[]): number {
    let count = 0;
    for (const toolId of toolIds) {
   ...
   */
  configureTool(toolId: ToolId, config: ToolConfigUpdate): boolean {
   ...
   */
  setPolicy(target: ToolId | ToolCategory, policy: ToolPolicy): void {
   ...
   */
  getPolicy(target: ToolId | ToolCategory): ToolPolicy | undefined {
   ...
   * @param target - Tool ID or category
   */
  removePolicy(target: ToolId | ToolCategory): boolean {
   ...
   */
  getConfig(toolId: ToolId): ToolConfig | undefined {
    const tool = getTool(toolId);
    return tool?.config;
  }
   ...
   */
  getEffectiveConfig(toolId: ToolId): ToolConfig {
    const tool = getTool(toolId);
    if (!tool) {
      return {
        enabled: false,
    ...
    const stored = tool.config || {} as ToolConfig;
  ...
  listTools(options?: {
    ...
    category?: ToolCategory;
  }): Array<{ definition: ToolDefinition; status: ToolStatus; config: ToolConfig }> {
   ...
   */
  setCategoryStatus(category: ToolCategory, enabled: boolean): number {
   ...
   */
  getRateLimitStatus(toolId: ToolId): { remaining: number; limit: number; resetAt: number } | undefined {
    const tool = getTool(toolId);
    if (!tool?.rateLimit) {
      return undefined;
    }
   ...
   */
  getCostTracking(toolId: ToolId): { spent: number; limit: number; remaining: number } | undefined {
    const tool = getTool(toolId);
    if (!tool?.cost) {
      return undefined;
    }
...
export const configureTool = (toolId: ToolId, config: ToolConfigUpdate): boolean =>
  toolManager.configureTool(toolId, config);

export const getToolConfig = (toolId: ToolId): ToolConfig | undefined =>
  toolManager.getConfig(toolId);

export const setToolPolicy = (target: ToolId | ToolCategory, policy: ToolPolicy): void =>
  toolManager.setPolicy(target, policy);

export const getToolPolicy = (target: ToolId | ToolCategory): ToolPolicy | undefined =>
  toolManager.getPolicy(target);

export const getToolsByCategory = (category: ToolCategory): ToolDefinition[] =>
  toolManager.getToolsByCategory(category);

export const setCategoryStatus = (category: ToolCategory, enabled: boolean): number =>
  toolManager.setCategoryStatus(category, enabled);

export const requiresApproval = (toolId: ToolId): boolean =>
  toolManager.requiresApproval(toolId);
