...
import { ToolInput, ToolOutput, ToolSchema, SchemaProperty, ValidationResult } from './types';
...
export const VALIDATION_MESSAGES = {
  required: (field: string) => `Required field '${field}' is missing`,
  type: (field: string, expected: string, actual: string) =>
    `Field '${field}' expected ${expected}, got ${actual}`,
  enum: (field: string, validValues: string[]) =>
  ...
  format: (field: string, format: string) =>
    `Field '${field}' is not a valid ${format}`,
  ...
  additionalProperties: (field: string) =>
    `Additional properties not allowed: '${field}'`,
};
...
export function validateProperty(
  ...
  if (property.type === 'string' && typeof value === 'string') {
    ...
    // Format validation
    if (property.format && VALIDATION_FORMATS[property.format as keyof typeof VALIDATION_FORMATS]) {
      const regex = VALIDATION_FORMATS[property.format as keyof typeof VALIDATION_FORMATS];
      if (!regex.test(value)) {
  ...
  if (Array.isArray(value) && property.type === 'array') {
    ...
    if (property.items) {
      ...
      value.forEach((item, index) => {
        const itemSchema = property.items as SchemaProperty;
 ...
 */
export function validateOutput(output: ToolOutput): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
 ...
 */
export function inferSchema<T>(example: T): ToolSchema {
  return {
    type: 'object',
 ...
 */
function inferProperties(obj: unknown): { [key: string]: SchemaProperty } {
 ...
 */
function inferProperty(value: unknown): SchemaProperty {
  if (value === null) {
    return { type: 'null' };
  }
 ...
 */
function typeMatches(expected: SchemaProperty['type'], actual: string): boolean {
  const typeMap: Record<SchemaProperty['type'], string[]> = {
    string: ['string'],
    number: ['number'],
    integer: ['number'],
    boolean: ['boolean'],
 ...
 */
export function createValidator(schema: ToolSchema) {
  return (input: ToolInput, options?: { sanitize?: boolean }): ValidationResult =>
