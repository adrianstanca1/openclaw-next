...
export class TaskAnalyst {
   ...
   */
  async analyze(userRequest: string): Promise<TaskAnalysis> {
    const availableTools = (skillRegistry as any).list().map((s: any) => s.id);
...
Respond in JSON:
{
  ...
  "complexity": "high"
    ...
    try {
      const analysis: TaskAnalysis = JSON.parse(response.choices[0].message.content);
      return analysis;
    } catch (e) {
      return {
