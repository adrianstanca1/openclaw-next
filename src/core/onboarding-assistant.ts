...
export interface SetupScheme {
  ...
  requiredKeys: string[];
  suggestedLocalModels: string[];
  capabilitiesToEnable: string[];
}
...
Explain your choices simply.
   ...
   */
  async proposeScheme(userGoal: string): Promise<SetupScheme> {
    const prompt = `
User Goal: "${userGoal}"
...
Respond in JSON format:
{
   ...
   */
  async getCredentialRequest(keyName: string): Promise<string> {
