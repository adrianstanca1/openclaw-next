...
import type { LLMRequest, LLMResponse, LLMProvider, ModelConfig } from './types.js';
...
export class ModelGateway {
  ...
  private localProvider: string = 'ollama';
  private forceLocal: boolean = true; // Priority: Local use
   ...
   */
  private analyzeComplexity(request: LLMRequest): 'low' | 'medium' | 'high' {
   ...
   */
  private getOptimalProvider(complexity: string, requestedModel: string): string {
    if (this.forceLocal) return this.localProvider;
   ...
   */
  setLocalPriority(enabled: boolean) {
