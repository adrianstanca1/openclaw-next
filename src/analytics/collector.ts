...
export class AnalyticsCollector {
   ...
   */
  recordSkillInvocation(agentId: string, skillId: string, durationMs: number): void {
   ...
   * Get metric series
   */
  getMetricSeries(name: string, startTime?: number, endTime?: number): MetricSeries | undefined {
    const series = this.metricSeries.get(name);
    if (!series) return undefined;

    if (startTime || endTime) {
      const filtered = series.data.filter(
        p => (!startTime || p.timestamp >= startTime) &&
   ...
   */
  getAggregatedStats(timeRange: '1h' | '24h' | '7d' | '30d'): {
    totalRuns: number;
    successRate: number;
    avgExecutionTime: number;
    topSkills: Array<{ skillId: string; count: number }>;
    topTools: Array<{ toolId: string; count: number }>;
    activeAgentsTrend: Array<{ timestamp: number; count: number }>;
  } {
    const now = Date.now();
    const rangeMs = {
      '1h': 60 * 60 * 1000,
  ...
  private recordRunCompletion(agentId: string, success: boolean): void {
    const metrics = this.agentMetrics.get(agentId);
    if (!metrics) return;
  ...
  private archiveAgentMetrics(agentId: string): void {
    // Could persist to storage here
    this.agentMetrics.delete(agentId);
  }

  private addMetricPoint(seriesName: string, value: number, labels: Record<string, string> = {}): void {
