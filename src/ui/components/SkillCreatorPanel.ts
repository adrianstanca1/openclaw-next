...
import type { SkillRequest, SkillAnalysisResult } from '../../skills/skill-creator.js';

@customElement('skill-creator-panel')
export class SkillCreatorPanel extends LitElement {
  static styles = css`
    :host {
      display: block;
      padding: 24px;
  ...
  @state() private request: SkillRequest = {
    description: '',
    purpose: '',
    targetUsers: [],
    expectedInputs: [],
  ...
  @state() private analysis: SkillAnalysisResult | null = null;
  @state() private activeTab = 'form';
  @state() private isAnalyzing = false;
  @state() private isRegistering = false;
  @state() private message: { type: 'success' | 'error'; text: string } | null = null;

  private skillCreator = new SkillCreator();
  ...
  private updateDescription(e: Event) {
    this.request = { ...this.request, description: (e.target as HTMLTextAreaElement).value };
  }

  private updatePurpose(e: Event) {
    this.request = { ...this.request, purpose: (e.target as HTMLInputElement).value };
  }

  private updateTargetUsers(e: Event) {
  ...
  private updateExpectedInputs(e: Event) {
  ...
  private updateConstraints(e: Event) {
    const value = (e.target as HTMLInputElement).value;
    this.request = {
      ...this.request,
