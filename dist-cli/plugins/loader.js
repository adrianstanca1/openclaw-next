...
export class PluginLoader {
     ...
     */
    async loadFromManifest(manifest, source = 'local', installPath, config) {
     ...
     */
    async loadFromDefinition(definition, source = 'bundled', installPath, config) {
     ...
     */
    async executeLoad(plugin, source, installPath, providedConfig) {
        const startTime = Date.now();
        const pluginId = 'id' in plugin ? plugin.id : plugin.id;
        const result = {
            success: false,
            warnings: [],
        };
        try {
            ...
            // Resolve config
            const resolvedConfig = this.resolveConfig('defaultConfig' in plugin ? plugin.defaultConfig : undefined, providedConfig);
     ...
     */
    resolveConfig(defaults, provided) {
 ...
 */
export const loadPlugin = (manifest, source, path, config) => pluginLoader.loadFromManifest(manifest, source, path, config);
export const loadPluginFromDefinition = (definition, source, path, config) => pluginLoader.loadFromDefinition(definition, source, path, config);
export const getLoadedPlugin = (instanceId) => pluginLoader.getLoaded(instanceId);
export const unloadPlugin = (instanceId) => pluginLoader.unload(instanceId);
